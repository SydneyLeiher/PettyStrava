...
Every runner has that one friend whose watch conveniently "glitches" right before the hill. Petty Strava puts a stop to the mystery. Petty Strava is all about transparency.
...
- **Finds real pauses, not slow patches.** It looks for gaps in the timestamp sequence itself — the difference between "ran a slow mile" and "watch stopped recording for 40 seconds" — so a bad hill doesn't get mistaken for a bathroom break.
...
3. Click the **Analysis** tab in the left sidebar (added by the extension).
4. Scroll to the **Data** section and click the **TCX** link next to "View: raw · graphs".
5. Drop that file into Petty Strava.

The app expects a standard TCX export with timestamped trackpoints — that's what step 4 gives you.
